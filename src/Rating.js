class Rating {
    ...
    // Convert keys to camelCase (e.g., "five_star" → "fiveStar")
    ...
    // Get distribution as percentages (e.g., { fiveStar: 50.0, ... })
    getDistribution() {
    ...
    // Render in specified format (html, svg, json)
    ...
    // Render HTML output (stars or bars)
    renderHtml(options) {
    ...
    // Map keys to display labels (e.g., "fiveStar" → "5-star")
    getStarLabels() {
        return {
            fiveStar: "5-star",
            fourStar: "4-star",
    ...
    // Render summary text (average + total)
    renderSummary() {
        return `
    ...
    // JS for interactive star ratings
    getInteractiveJS() {
        return `
      <script>
        (function() {
    ...
    renderSvg(options = {}) {
        ...
        // SVG star path (golden ratio star)
    ...
    // JS for interactive SVG ratings
    getSvgInteractiveJS() {
        return `
        <script>
